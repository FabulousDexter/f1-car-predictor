...
def get_all_event_schedule(start_year: int, end_year: int = None):
    """
    Get all scheduled F1 events for the specified years.
    Returns ALL events, including future/incomplete ones.
    Filtering for completed races is done in data cleaning.
    ...
    Returns:
        DataFrame: All scheduled F1 events
    """
    if end_year is None:
    ...
    print(f"\nFound {len(all_events)} events from {start_year} to {end_year}")
...
def get_all_sessions(
    ...
    if all_results:
        ...
        save_dateframe_to_csv(results_df, f"data/f1_all_session_race_results.csv")
        save_dateframe_to_csv(laps_df, f"data/f1_all_session_race_laps.csv")
    else:
